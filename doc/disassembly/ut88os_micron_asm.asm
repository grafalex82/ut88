...
; facilities, compared to built-in assembler.
;
;
; Assembler syntax
; ----------------
;
; The Assembler processes input text in the 0x3000-0x9fff range, and produce byte code at 0xa000+ space by default.
;
; Lines that start with a ';' (semicolon) are comments, and just ignored
;
; Each line of the source code must follow this syntax:
; [<label>:] <instruction> [arguments]
;
; Example:
; LBL: MOV A, B
;
; Label is an identifier up to 6-chars length, starting with a letter. It is possible to use labels in instruction
; arguments, or value expressions. It is also possible to use labels that are defined later in the code.
;
; Example:
; JMP LBL
; or
; CALL LBL+5
;
; Named value may be added to the code using EQU directive:
; <name>: EQU <value>
;
; Name follows the same requirements as a label (up to 6 chars, starting with letter).
;
; Example:
; MYVALUE: EQU 5
;
; Instruction immediate values may be one of the following types:
; - 1234    - Decimal value
; - 5AH     - Hexadecimal value (must start with a decimal digit, and end with 'H')
; - 'A'     - 1-char symbol (for instructions that have 1-byte immediate argument)
; - 'BC'    - 2-char symbol literal (for instructions that have 2-byte immediate argument)
; - $       - reference to the current instruction address
; - label references and EQU values may be also used in the code
; 
; The immediate values may be an expression, that is a sum or difference of several elements of types above.
; Unary minus or plus is also possible for the first member.
;
; Examples:
; - MVI A, '0'
; - LXI DE, 1234H
...
; 
;
; DB and DW directives may be used to add raw data to the program (bytes or words respectively). Both directives
; accept comma separated list of expressions (format described above). The only addition is that DB directive
; supports strings. Strings are not NULL terminated.
;
; Examples:
; - DB 1, 2, 3, 'Q'
; - DB 'LONG STRING'
;
;
; There is no way to change input source location, but there are some directives to control the output. It worth
; noting first, that there are 2 types of output addresses:
; - Target storage location - where the compiled bytes will be stored to. By default the compiler will produce
;   bytes at 0xa000+ memory range, but this may be altered with DS directive (which offsets the storage location
;   a little bit). For example, suppose you want the code to be produced at 0xa100 address. In this case use
;   'DS 100H' directive to shift the storage location by 0x100 bytes.
; - Target execution adress - where the compiled code will be eventually executed. By default this address matches
;   the storage location (0xa000), but can be changed with ORG directive. Suppose you are compiling the code that
;   would run at 0x1000 memory range. Using 'ORG 1000H' directive the compiled code will be still placed at 0xa000,
;   but this byte code can be then memcopied to 0x1000 where it will run normally.
;
;
; Usage
; -----
...
; - '2' - verbose mode. The compiler dumps the source code, and annotates every line with the obtained target
;         address, byte code generated, EQU and label values.
...
; MOV A,B
;
; The program is 2 bytes long. It is compiled to 0xa000 address space, but supposed to run from 0x100 address.
; The report will look as follows:
;
; ERRORS DETECTED:00
; 0101/A001/
;
; Where 0x0101 is the last byte in execution address range, and 0xa001 is the last byte in the storage range.
; 
; In case of errors, they will be counted to be reported in the summary. More detailed error information can be
; obtained in #2 verbose working mode. 
;
...
; 
; The first field (if exist) indicate an error. The number is a bitmask of possible errors:
...
; ----------------------------------
;
; Parsing and compiling the source code is done on line-by-line basis. Each line is copied to the line buffer
; so that all the function are working with a copy. In turn, the line is parsed literal by literal. The literal
; is a number or a word up to 6 chars wide. Each literal is copied to the working buffer, so that matching 
; functions work with that buffer.
;
; Labels are stored in the labels table. This table is located right after the last source text symbol. Each 
; record consists of label text (6 chars) and 16-bit label value. There are functions to add a label to the table,
; as well as search the label record.
;
...
; - This program expects 0x01-0x1f keycodes for Ctrl combinations. This matches the original UT-88 Monitor F,
;   but does not match UT-88 OS Monitor.
;
; Variables:
...
; - bf84 - error bitmask (each bit indicates a certain type of error)
; - bf85 - current output pointer (starting 0xa000). Advances within multibyte instruction.
; - bf87 - current instruction target address. Stays fixed while instruction is being parsed.
...
; - bf8f - currently processing line pointer (in the source text)
...
GET_KBD_INPUT:
    ...
    d82b  ca 65 f8   JZ MONITOR_RESTART_MAIN_LOOP (f865)    ; Hack/bug: use direct call to Monitor, not 0xf803
...
; Parse/Compile a single line
;
; Algorithm:
; - The line is copied from the input text into the line buffer.
; - parse a label definition (if any)
; - Parse instruction mnemonic or match compiler directive
; - Parse instruction argument (if any)
...
PROCESS_NEXT_LINE_1:
    ...
    d8b1  7e         MOV A, M                   ; Load expected bytes count for this type of instruction
    
    d8b2  b9         CMP C                      ; Compare it with bytes count calculated by PARSE_EXPRESSION
...
ADVANCE_TO_NEXT_LINE:
    d8c4  cd 80 dd   CALL DUMP_LINE (dd80)      ; Dump the bytecode associated with the line and source line
...
; The function above parses the instruction mnemonic, and the first argument. All other processing is done
; with a respective handler, depending on the instruction type.
...
OUTPUT_HANDLER_1B_MOV:
    ...
    d8de  cd 9a db   CALL PARSE_2ND_ARG (db9a)  ; Destination register is parsed by now, parse source register
...
OUTPUT_HANDLER_2B_MVI:
    ...
    d8ec  cd 9a db   CALL PARSE_2ND_ARG (db9a)      ; Destination register is parsed earlier. Parse the immediate
...
OUTPUT_HANDLER_2B_IMMEDIATE:
    ...
    d8f1  c3 40 da   JMP STORE_IMMEDIATE_ARG (da40) ; The value itself is parsed during parsing the instruction
...
OUTPUT_HANDLER_3B_LXI:
    ...
    d8fc  cd 9a db   CALL PARSE_2ND_ARG (db9a)      ; Parse immediate argument
...
; DS compiler directive handler
;
; The function advances the storage output pointer by a value provided as directive argument
DS_HANDLER:
...
EQU_HANDLER:
    ...
    d949  3a 84 bf   LDA ERROR_BITMASK (bf84)           ; Do not perform EQU processing if the label part was not
    d94c  3d         DCR A                              ; properly matched.
    d94d  c8         RZ
...
DB_DW_HANDLER_STORE:
    ...
    d994  cd 9a db   CALL PARSE_2ND_ARG (db9a)      ; Parse the next item
...
PRINT_ERROR_SUMMARY:
    ...
    da14  0e 2f      MVI C, 2f                      ; Print program last byte address in target execution 
    da16  cd 48 de   CALL PRINT_WORD_HEX (de48)     ; address range, followed by '/'
...
; Store instruction opcode to output
; The function combines the instruction opcode with the register bits, and stores it to the output.
...
; 
; Return number suggested size of the instruction in C, or 0xff in case of error
PARSE_IMMEDIATE_VALUE:
    ...
    dc70  11 e0 bf   LXI DE, bfe0               ; Start parsing from the beginning of the working buffer
    dc73  0e 00      MVI C, 00                  ; C will represent number of parsed chars
...
PARSE_MNEMONIC_1:
    ...
    dd1b  d6 41      SUI A, 41                  ; Symbols with codes < 'A' are invalid. Report a match error
    ...
    dd3f  91         SUB C                      ; Symbol below 0x40 (letter) is a syntax error
...
; - number of data bytes expected to be returned by function that parses the first argument (this may include
;   register bit masks, data bytes, etc. This correlate but does not strictly match to the instruction length)
OUTPUT_HANDLERS_TABLE:
    ...
    de6d  d8 00     ; arg type 0x0d: END compiler directive (END_HANDLER)
...
MNEMONIC_2ND_3RD_LETTER_LUT:
    ...
    deba  21 01 80                      ; ADD (base opcode 0x80, type=0x01 - source register coded in opcode)
